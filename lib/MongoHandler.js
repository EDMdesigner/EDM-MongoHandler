...
module.exports = function MongoHandler(modelToWorkOn, returnModeToSendBack) {
	...
	function find(req, res, optionsParam, logMessage, callback) {
		...
		function onCounted(err, count) {
			if(err) {
				...
				onFinished(returnObject, callback, req, res, logMessage, "find");
			} else {
				totalCount = count;
	...
	function findOne(req, res, find, select, callback, logMessage) {
		...
		function onFound(err, result) {
			...
			onFinished(returnObject, callback, req, res, logMessage, "findOne");
		}
	}
	...
	function count(req, res, find, callback, logMessage) {
		...
		function onFound(err, result) {
			...
			onFinished(returnObject, callback, req, res, logMessage, "count");
		}
	}
	...
	function remove(req, res, find, callback, logMessage) {
		...
		function onExecuted(err, result) {
			...
			onFinished(returnObject, callback, req, res, logMessage, "remove");
		}
	}
	...
	function removeOne(req, res, find, callback, logMessage) {
		...
		function onExecuted(err, result) {
			...
			onFinished(returnObject, callback, req, res, logMessage, "removeOne");
		}
	}
	...
	function create(req, res, item, callback, logMessage) {
		...
		function onSaved(err, result) {
			...
			onFinished(returnObject, callback, req, res, logMessage, "create");
		}
	}
	...
	function createMore(req, res, items, callback, logMessage) {
		...
		async.forEach(items,
					function(item, callback) {
						...
						onFinished(returnObject, callback, req, res, logMessage, "createMore");
					});
	...
	function updateOne(req, res, find, updateObject, callback, logMessage) {
		...
		function onItemFound(err, item) {
			if(err) {
				returnObject.err = err;
				onFinished(returnObject, callback, req, res, logMessage, "updateOne");
			} else if(item){
		...
		function onFound(err, items) {
			...
			async.forEach(items,
				function(item, callback) {
					...
					onFinished(returnObject, callback, req, res, logMessage, "updateMore");
				});
		}
	}
	...
	function duplicateOne(req, res, find, callback, logMessage) {
		...
		function onFound(err, item) {
			if(err) {
				returnObject.err = err;
				onFinished(returnObject, callback, req, res, logMessage, "duplicateOne");
				return;
			} else {
				if(!item) {
					returnObject.err = 'Not found';
	...
	function onFinished(returnObject, callback, req, res, logMessage, calledFrom) {
		...
		if(returnObject.err && returnObject.err !== "Not found") {
			...
			errorLogObject.statusText = "Error: " + returnObject.err.toString() + " -  Stack: " + new Error(errorMsg).stack;
			if(req && req.headers) {
